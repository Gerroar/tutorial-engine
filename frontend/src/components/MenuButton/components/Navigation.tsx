...
function generateTsx(marginL: number, marginR: number, name: string, index: string, hasDocsInside: boolean, currentPageIndex: number, setCurrentPageIndex: any, fileCountIndex: number, setFileCountIndex: any) {

  if (index.length === 1) {

    return (<MenuItem marginL={`${marginL}rem`} marginR={`${marginR}rem`} directoryElm={name} elementIndex={index} key={index} hasDocsInside={hasDocsInside} hidden={false} currentPageIndex={currentPageIndex} setCurrentPageIndex={setCurrentPageIndex} fileCountIndex={fileCountIndex} setFileCountIndex={setFileCountIndex} />)
  } else {

    return (<MenuItem marginL={`${marginL}rem`} marginR={`${marginR}rem`} directoryElm={name} elementIndex={index} key={index} hasDocsInside={hasDocsInside} hidden={true} currentPageIndex={currentPageIndex} setCurrentPageIndex={setCurrentPageIndex} fileCountIndex={fileCountIndex} setFileCountIndex={setFileCountIndex} />)
  }
...
export const Navigation = ({ currentPageIndex, setCurrentPageIndex }: { currentPageIndex: number, setCurrentPageIndex: any }) => {

  const [fileCountIndex, setFileCountIndex] = useState(0);

  if (flattenOutput.length === 0) {

    return (
      <motion.ul variants={variants} className="ul-menu" id="ul-menu">
        <MenuItem marginL="0rem" marginR="0rem" directoryElm={"No files or directories"} elementIndex={"1"} key={"1"} hasDocsInside={false} hidden={false} currentPageIndex={currentPageIndex} setCurrentPageIndex={setCurrentPageIndex} fileCountIndex={fileCountIndex} setFileCountIndex={setFileCountIndex} />
      </motion.ul>
    )
  } else {
      ...
      <motion.ul variants={variants} className="ul-menu" id="ul-menu">
        {
          flattenOutput.map(value => {
            ...
            if (index.length === depth) {
              ...
              return generateTsx(marginL, marginR, name, index, hasDocsInside, currentPageIndex, setCurrentPageIndex, fileCountIndex, setFileCountIndex)
